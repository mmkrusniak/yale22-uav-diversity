...
/**
 * This is a legacy class which, as far as I can tell, is intended to better model the drone's energy usage in
 * straight lines.
 */
public class Path {
    ...
    public Path(List<Point> wayPoints, int index, int endIndex){
...
//            }
            length += wayPoints.get(i-1).distance2D(wayPoints.get(i));
        }
        return length;
    }
...
//            }
    ...
    public double getDecelarationDistance(){
        ...
        PolynomialFunction f = new PolynomialFunction(vector);
        UnivariateFunction uf = new PolynomialFunction(vector);
    ...
    public List<Double> getSpeed() {
        ...
        for(int i=startIndex;i<indexOfWayPointEndAcc;i++){
                speedList.add(Math.sqrt(2* Option.acceleration *distancePerWaypoints.get(i-startIndex)));
            }
        ...
        if (indexOfWayPointEndAcc<=indexOfWayPointStartDec){
            ...
            for(int i=indexOfWayPointStartDec+1;i<=endIndex;i++){
                speedList.add(Math.sqrt(2*Option.deceleration *(getLength()-distancePerWaypoints.get(i-startIndex))));
            }
